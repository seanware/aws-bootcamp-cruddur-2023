...
class ReplyToActivityUuidToStrinMigration:
  def migrate_sql():
    data = """
  ...
  def rollback_sql():
    ...
    return data
  def migrate():
    db.query_commit(ReplyToActivityUuidToStrinMigration.migrate_sql(),{
    })
  def rollback():
    db.query_commit(ReplyToActivityUuidToStrinMigration.rollback_sql(),{
    })
